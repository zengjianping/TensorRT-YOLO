...
def process_image(sess, im_pil):
    ...
    im_data = transforms(resized_im_pil)
    im_data = torch.stack([im_data] * num_images)
    ...
    result_images = draw(
        [im_pil] * num_images, num_dets, labels, boxes, scores,
        [ratio] * num_images, [(pad_w, pad_h)] * num_images
    )
